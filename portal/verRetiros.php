                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title">Retiros de caja</h4>
                            <a href="agregarRetiro.php"><button type="button" class="btn btn-social-icon-text btn-add"><i class='bx bx-plus'></i>Nuevo retiro de caja</button></a>
                        </div>
                        ...
                        <div class="table-responsive overflow-hidden" id="tabla">
                            <table id='dtEmpresa' class='table table-striped'>
                                ...
                                <tbody>
                                    ...
                                    $nivel == 1 ? $flsucursal = "" : $flsucursal = " AND tr_retiros.fk_sucursal = $pk_sucursal";

                                    $eretiro = "SELECT tr_retiros.*,
                                        ct_retiros.nombre as retiro,
                                        ct_sucursales.nombre as sucursal
                                    FROM tr_retiros, ct_sucursales, ct_retiros
                                    WHERE tr_retiros.estado = 1$flsucursal
                                    AND ct_sucursales.pk_sucursal = tr_retiros.fk_sucursal
                                    AND tr_retiros.fk_retiro = ct_retiros.pk_retiro";

                                    if (!$resultado = $mysqli->query($eretiro)) {
                                    ...
                                    while ($row = $resultado->fetch_assoc()) {
                                        ...
                                        echo <<<HTML
                                            <tr class='odd gradeX'>
                                                ...
                                                <td>$row[sucursal]</td>
                                                <td><i class='bx bx-user fs-5'></i> $row[fk_usuario]</td>
                                                <td>$row[retiro]. $row[descripcion]</td>
                                                <td><p class='badge-danger-integra'>$$monto</p></td>
                                                <td><a target='_blank' href='retiroPDF.php?id=$row[pk_retiro]'><i class='fa fa-file-pdf-o vpdf fa-lg btn-pdf'></i></a></td>
                                            </tr>
                                        HTML;
                                    }
                                    ?>
