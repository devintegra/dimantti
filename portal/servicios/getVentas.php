...
//FILTROS
#region
$fk_sucursal != 0 ? $flsucursal = " AND tr_ventas.fk_sucursal = $fk_sucursal" : $flsucursal = "";

//$nivel == 3 || $nivel == 4 ? $flusuario = " AND trv.fk_usuario = '$fk_usuario'" : $flusuario = "";

if ($search_value != '') {
    $flsearch = " AND (tr_ventas.fecha LIKE '%$search_value%' OR tr_ventas.folio LIKE '%$search_value%' OR ct_clientes.nombre LIKE '%$search_value%' OR ct_sucursales.nombre LIKE '%$search_value%' OR tr_ventas.fk_usuario LIKE '%$search_value%' OR tr_ventas.observaciones LIKE '%$search_value%' OR tr_ventas.total LIKE '%$search_value%')";
}
...
if (!empty($column_search[3])) {
    $flsucursalnom = " AND ct_sucursales.nombre LIKE '%" . $column_search[3] . "%'";
}

if (!empty($column_search[4])) {
    $flcliente = " AND ct_clientes.nombre LIKE '%" . $column_search[4] . "%'";
}
...
while ($row = $rregistros->fetch_assoc()) {
    ...
    $sucursal = $row['sucursal'] . '(' . $row['almacen'] . ')';
    ...
    if (!$rseries = $mysqli->query($qseries)) {
        echo "<br>Lo sentimos, esta aplicación está experimentando problemas.";
        exit;
    }

    $series = $rseries->fetch_assoc();
    $series_pendientes = $series["pendientes"];
    #endregion
    ...
    if ($row['modificada'] == 0 && $row['estatus'] < 2 && ($nivel == 1 || $nivel == 4)) {
        $btn_acciones = $btn_acciones . "
            ...
            <button type='button' class='btn-reabrir-dast cancelar' title='Cancelar venta'>
