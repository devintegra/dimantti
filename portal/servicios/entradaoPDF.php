...
class PDF_Invoice extends FPDF
{
    ...
    function addQRI($imagen, $ancho, $alto)
    {
        $x1 = 1;
        $y1 = 4;
    ...
    function addQRIO($imagen, $ancho, $alto)
    {
        $x1 = 1;
        $y1 = 5;
    ...
    function addSingCarta($nom)
    {
        ...
        $length = $this->GetStringWidth("Lote: " . $nom);
        $this->Cell($length, 2, "Lote: " . $nom);
    }


    function addInfoCarta($nom)
    {
    ...
    function addInfoCartaExtra($nom)
    {
    ...
    function addDate($date)
    {
        ...
        $y2  = $y1;
        $mid = $y1 + ($y2 / 2);
        $this->RoundedRect($r1, $y1, ($r2 - $r1), 15, 3.5, 'D');
    ...
    function addProveedor($mode)
    {
        ...
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 1);
        $this->SetFont("Arial", "B", 10);
        $this->Cell(10, 4, "Proveedor Nombre", 0, 0, "C");
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 5);
        $this->SetFont("Arial", "", 10);
        $this->Cell(10, 5, $mode, 0, 0, "C");
    }

    function addAlmacenes($mode)
    {
        $mode = utf8_decode($mode);
        $r1  = 5;
        $r2  = $r1 + 53;
        ...
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 1);
        $this->SetFont("Arial", "B", 10);
        $this->Cell(10, 4, "Almacenes", 0, 0, "C");
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 5);
        $this->SetFont("Arial", "", 10);
        $this->Cell(10, 5, $mode, 0, 0, "C");
    }
    ...
    function addClienteCotizacion($mode)
    {
        $mode = utf8_decode($mode);
        $r1  = 5;
        $r2  = $r1 + 53;
        ...
        $this->SetXY($r1 + ($r2 - $r1) / 2, 37 + 5);
        $this->SetFont("Arial", "", 10);
        $this->Cell(10, 5, $mode, 0, 0, "C");
    }

    function addClientev($mode)
    {
        $mode = utf8_decode($mode);
        $r1  = 5;
        $r2  = $r1 + 53;
    ...
    function addProveedorTel($mode)
    {
        ...
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 1);
        $this->SetFont("Arial", "B", 10);
        $this->Cell(10, 4, "Proveedor Telefono", 0, 0, "C");
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 5);
        $this->SetFont("Arial", "", 10);
        $this->Cell(10, 5, $mode, 0, 0, "C");
    }

    function addClientevTel($mode)
    {
        $mode = utf8_decode($mode);
        $r1  = 50;
        $r2  = $r1 + 53;
    ...
    function addProveedorCorreo($mode)
    {
        $mode = utf8_decode($mode);
        $r1  = 92;
        $r2  = $r1 + 53;
        ...
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 1);
        $this->SetFont("Arial", "B", 10);
        $this->Cell(10, 4, "Proveedor Correo", 0, 0, "C");
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 5);
        $this->SetFont("Arial", "", 10);
        $this->Cell(10, 5, $mode, 0, 0, "C");
    }
    ...
    function addClientevCorreo($mode)
    {
        $mode = utf8_decode($mode);
        $r1  = 92;
        $r2  = $r1 + 53;
    ...
    function addObservacionesSalidas($mode)
    {
        $mode = utf8_decode($mode);
        $r1  = 5;
        $r2  = $r1 + 53;
        ...
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 1);
        $this->SetFont("Arial", "B", 10);
        $this->Cell(10, 4, "Motivo de salida", 0, 0, "C");
        $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, 37 + 5);
        $this->SetFont("Arial", "", 10);
...
*/
    ...
    function addColsInventario($tab, $y)
    {
        global $colonnes;
...
*/
    ...
    function addLineFont($ligne, $tab, $fontweight, $fontsize)
    {
        global $colonnes, $format;
        ...
        reset($colonnes);
        $this->SetFont("Arial", $fontweight, $fontsize);
        foreach ($colonnes as $lib => $pos) {
            $longCell = $pos - 2;
            $texte = $tab[$lib];
            $length = $this->GetStringWidth($texte);
    ...
    function addSubtotalCona($total)
    {
        ...
        $this->SetXY($x1 + 20, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addEfectivoEntrada($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Efectivo (ingreso)', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addEfectivoSalida($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Efectivo (egreso)', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addTransferenciaEntrada($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Transferencia (ingreso)', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addTransferenciaSalida($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Transferencia (egreso)', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addTarjetaEntrada($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, utf8_decode('Tar.Crédito (ingreso)'), 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addTarjetaSalida($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, utf8_decode('Tar.Crédito (egreso)'), 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addTarjetaDebitoEntrada($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Tar.Debito (ingreso)', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addTarjetaDebitoSalida($total)
    {
    ...
    function addChequeEntrada($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Cheque (ingreso)', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addChequeSalida($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Cheque (egreso)', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addCorteTotal($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 8, 'Total MXN', 1, 0, 'C');
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(40, 8, $total, 1, 0, 'R');
    }

    function addTotalGeneral($total, $x, $y)
    {
    ...
    function addCantidades($total, $x, $y, $h)
    {
        ...
        $this->SetXY($x1 + 30, $y1);
        $this->SetFont('Arial', 'B', 12);
        $this->Cell($h, 8, $total, 1, 0, 'R');
    }

    function addTotalVentaa($total)
    {
    ...
    function addTotalVentaab($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(28, 8, 'Total', 1, 0, 'C');
        $this->SetXY($x1 + 28, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(56, 8, $total, 1, 0, 'R');
    }


    function addTotalVentaad($total)
    {
    ...
    function addDescuentoCon($total)
    {
        ...
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 8, 'Pago', 1, 0, 'C');
        $this->SetXY($x1 + 20, $y1);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(20, 8, $total, 1, 0, 'R');
    }

    function addDescuentoCona($total)
    {
    ...
    function addTotalVentaCon($total)
    {
    ...
    function addTotalVentaCona($total)
    {
