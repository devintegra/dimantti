...
while ($row = $rsp_get_productos->fetch_assoc()) {
    ...
    if ($codigo == 200) {
        $mysqli->next_result();
        if (!$mysqli->query("CALL sp_update_existencias_salida_apartado($fk_sucursal, $fk_almacen, $row[fk_producto], $row[cantidad])")) {
            $codigo = 201;
    ...
    if ($codigo == 200) {
        $mysqli->next_result();
        if (!$mysqli->query("INSERT INTO tr_movimientos (fk_producto, fk_movimiento, fk_movimiento_detalle, fk_usuario, fk_sucursal, fk_almacen, tipo_venta, fecha, serie, cantidad, total) VALUES($row[fk_producto], 2, $pk_venta, '$fk_usuario', $fk_sucursal, $fk_almacen, 0, CURDATE(), '', $row[cantidad], $row[total])")) {
            $codigo = 201;
